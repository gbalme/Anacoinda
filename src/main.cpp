...
const string strMessageMagic = "Anacoinda Signed Message:\n";
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Firstblock premine
    if (nHeight == 1)
        return 60 * 10 * 1000 * COIN + nFees;

    // First 90 days subsidy 200*day coins
    if (nHeight <= 32400)
    {
        int64 p = ((50 * COIN) + (10000 * COIN / nHeight)) + nFees;
        return p;
    }
...
static const int64 nTargetTimespan = 5 * 60; // 5mn
static const int64 nTargetSpacing = 60; // 1mn
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis block
        const char* pszTimestamp = "almost february 2014";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        //for sha2
        //block.nTime    = 1365638401;
        //block.nNonce   = 1627205376;

        // for sha3
        block.nTime    = 1391127370;
        block.nNonce   = 0;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AnacoindaMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AnacoindaMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("AnacoindaMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);    

    RenameThread("Anacoinda-miner");
    ...
    while (fGenerateBitcoins)
    {
        ...
        printf("Running AnacoindaMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try
    {
        ...
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadAnacoindaMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "AnacoindaMiner()");
    }
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadAnacoindaMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d AnacoindaMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!NewThread(ThreadBitcoinMiner, pwallet))
                printf("Error: NewThread(ThreadAnacoindaMiner) failed\n");
            Sleep(10);
        }
    }
}
